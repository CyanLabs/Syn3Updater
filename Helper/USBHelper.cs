                ...
                try
                {
                    foreach (ManagementBaseObject o in partitionQuery.Get())
                    {
                        ...
                        foreach (ManagementBaseObject managementBaseObject in logicalDriveQuery.Get())
                        {
                            ...
                            drive_info.FileSystem += $" {Convert.ToString(ld.Properties["FileSystem"].Value).ToString()}";
                            drive_info.Name = ld.Properties["VolumeName"].Value.ToString();
                            
                            if (drive_info.FileSystem == "exFAT" &&
                                drive_info.PartitionType == "MBR" &&
                                drive_info.Name == "CYANLABS") drive_info.SkipFormat = true;
                        }
                    }
                }
